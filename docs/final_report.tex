...
The training set contains 1,604 labeled samples, and the test set contains 8,424 samples. Each sample has two SAR bands of size $75 \times 75$. There are 133 missing incidence-angle values in the training set and no missing incidence-angle values in the test set. The training labels are only mildly imbalanced: 851 ship samples and 753 iceberg samples, corresponding to approximately 53.1\% ship and 46.9\% iceberg.
...
\subsection{Training settings and hyperparameter tuning}

Hyperparameter tuning focused on stable validation performance. We avoided maximizing training-set fit and used the final reproduction settings below.

\begin{center}
\begin{tabular}{p{0.28\linewidth}p{0.62\linewidth}}
\toprule
CNN component & Training settings \\
\midrule
Base A / Base C CNN & AdamW optimizer, learning rate 0.001, weight decay $10^{-4}$, batch size 64, epoch limit 120, early-stopping patience 25, width 48, dropout 0.20 \\
Base B VGG-style CNN & AdamW optimizer, learning rate 0.001, weight decay $10^{-4}$, batch size 64, epoch limit 120, early-stopping patience 25, width 48, dropout 0.30 \\
Base D FiLM ResNet34 & AdamW optimizer, learning rate 0.0003, weight decay $10^{-4}$, batch size 32, epoch limit 80, early-stopping patience 15, dropout 0.30 \\
...
\begin{tabular}{p{0.28\linewidth}p{0.62\linewidth}}
\toprule
Stacking component & Settings \\
\midrule
Pseudo-label filtering & High-confidence test predictions only: probability $\leq 0.03$ or $\geq 0.97$, pseudo-label weight 0.35, with raw \texttt{inc\_angle} decimal-precision filtering \\
...
\midrule
\texttt{submission\_blend\_8models\_20260503\_212025.csv} & \textbf{0.08098} \\
\bottomrule
\end{tabular}
\end{center}
...
\centering
\includegraphics[width=\linewidth]{figures/kaggle_private_top5.png}
\vspace{0.4em}

\includegraphics[width=\linewidth]{figures/kaggle_final_submission.png}
\vspace{0.3em}

\refstepcounter{figure}
\small \textbf{Figure \thefigure:} Kaggle evidence. Top: private Top-5 threshold (fifth place = 0.08883). Bottom: our strict late submission (private = 0.08098; public shown as supplementary).
\end{minipage}
\end{center}
\vspace{0.7em}
...
\begin{enumerate}
  ...
  \item \textbf{Code review and reproducibility checks.} AI helped review selected code snippets and inspect the cross-validation protocol, pseudo-label filtering, second-stage stacking, README, and one-command reproduction script. All core implementation was written, tested, and verified by the group.
...
The exact CSV file uploaded to Kaggle is included in the GitHub repository under:

\begin{lstlisting}
submissions/submission_blend_8models_20260503_212025.csv
\end{lstlisting}

Re-running the reproduction script generates a timestamped CSV under:

\begin{lstlisting}
predictions/submission_blend_8models_*.csv
\end{lstlisting}

The filename itself does not affect Kaggle scoring, but we keep the original name in the report so it matches the Kaggle submission record.
